...
echo "    <Appenders>"
echo "        <Console name=\"Console\" target=\"SYSTEM_OUT\"> <PatternLayout pattern=\"%d %p %m%n\" /> </Console>"
...
echo "    </Appenders>"
echo "    <Loggers>"
echo "        <Root level=\"trace\"> <appender-ref ref=\"Console\" />   </Root>"

for METHOD in "${METHODS[@]}"
do
    echo "        <logger name=\"$METHOD\" level=\"debug\"> <appender-ref ref=\"$METHOD\" /> </logger>"
done
